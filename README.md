# Snowball - OKX Adaptive Grid Trading Agent

OKX 거래소에서 동작하는 적응형 그리드 트레이딩 봇입니다. 시장 변동성을 실시간 분석하여 그리드 간격을 자동 조절하고, 위험 상황에서는 Claude AI의 판단을 받아 대응합니다.

## 작동 방식

30초마다 아래 사이클을 반복합니다:

1. **시장 데이터 수집** - OKX API에서 캔들 데이터 조회
2. **리스크 스코어 산출** (0~100) - ATR, RSI, 볼린저밴드, 거래량 4개 지표 종합
3. **상태 결정 및 액션 실행**
4. **텔레그램 알림** (상태 변화 시)

### 상태 머신

| 스코어 | 상태 | 액션 |
|--------|------|------|
| 0~30 | NORMAL | 그리드 유지 |
| 31~60 | CAUTION | 그리드 간격 확대 |
| 61~80 | WARNING | 신규 주문 중단 |
| 81~100 | EMERGENCY | 전체 청산 |

스코어가 애매한 구간(55~80)에서는 Claude API에 판단을 위임합니다.

### 리스크 스코어 구성

| 지표 | 최대 점수 | 설명 |
|------|-----------|------|
| ATR | 30점 | 변동성 급등 감지 |
| RSI | 25점 | 과매수/과매도 극단값 |
| Bollinger Band | 25점 | 밴드 폭 급팽창 |
| Volume | 20점 | 거래량 급등 |

## 설치

```bash
pip install -r files/requirements.txt
```

## 설정

`files/config.py`에서 아래 값을 수정합니다:

```python
# 필수
OKX_API_KEY    = "your_api_key"
OKX_SECRET_KEY = "your_secret_key"
OKX_PASSPHRASE = "your_passphrase"
LLM_API_KEY    = "your_anthropic_key"

# 선택 (텔레그램 알림)
TELEGRAM_TOKEN   = "your_bot_token"
TELEGRAM_CHAT_ID = "your_chat_id"

# 거래 설정
SYMBOL       = "BTC-USDT"
TOTAL_BUDGET = 1000.0
DEMO_MODE    = True  # True = 모의거래, False = 실거래
```

## 실행

```bash
cd files
python main_agent.py
```

`Ctrl+C`로 종료합니다.

## 파일 구조

```
files/
├── config.py            # 설정값 (API 키, 거래 파라미터, 임계값)
├── main_agent.py        # 메인 루프, 상태 머신, LLM 판단, 텔레그램 알림
├── market_analyzer.py   # ATR/RSI/BB/거래량 분석 → 리스크 스코어 산출
├── grid_controller.py   # OKX Grid Bot API 제어 (시작/확대/중단/청산)
└── requirements.txt     # 의존성
```

## 주의사항

- `DEMO_MODE = True` 상태에서 충분히 테스트한 후 실거래로 전환하세요
- `config.py`에 API 키가 포함되므로 절대 공개 저장소에 올리지 마세요
- `MAX_LOSS_PERCENT` (기본 15%)에 도달하면 자동 청산됩니다
