...
2분 간격 루프 기준 (720회/일, 21,600회/월)이다람쥐.
리스크 스코어 55~80 구간에서만 호출하며, 수수료 컨텍스트 포함 **입력 ~1,000토큰 / 출력 ~100토큰** 기준이다람쥐.
멀티 에이전트 모드는 1회 판단에 **5회 호출** (에이전트 4명 + 조율자 1명)하는 다람쥐.

**멀티 에이전트 모드 (기본)** — 월간 비용

| 모델 | 1회 판단 | 안정장 (~5%) | 보통 (~15%) | 변동장 (~30%) |
|------|:--------:|:-----------:|:----------:|:------------:|
...
> 🏆 **가성비 추천** (멀티 에이전트 모드, 변동장 기준)
> - **Gemini 2.0 Flash**: 월 $3.24 — 최저가
> - **GPT-4o Mini**: 월 $6.48 — 가성비 최강
> - **Grok 3 Mini**: 월 $11.66
>
> 🧠 **판단 품질 추천**
> - **Claude Sonnet 4**: 보통장 월 $72.90
> - **GPT-4.1**: 보통장 월 $45.36 — 가성비 + 품질 균형
>
> 💰 **극한 절약**: `MULTI_AGENT_MODE=false` + Gemini 2.0 Flash = 변동장에서도 **월 $0.65**
